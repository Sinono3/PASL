...
from FR_Pretrained_Test.Model.model_lightcnn import (
    LightCNN_9Layers,
    LightCNN_29Layers,
...
def compute_g_loss(
        ...
        if args.pix2pix:
            ...
            for num in range(6):
                if num == 0:
                    ...
                    loss_fm_1 += torch.mean(
                        torch.abs(fake_fea_1[num] - real_fea_1[num])
                    )
                    loss_fm_2 += torch.mean(
                        torch.abs(fake_fea_2[num] - real_fea_2[num])
                    )
    ...
    if args.landmark_loss:
        ...
        for i in range(len(x_fake)):
            img = np.array(
                (x_fake[i].cpu().detach().numpy().squeeze().transpose(1, 2, 0)) * 256,
                dtype=np.uint8,
            )
        ...
        for i in range(len(x2_target)):
            img = np.array(
                (x2_target[i].cpu().detach().numpy().squeeze().transpose(1, 2, 0))
                * 256,
                dtype=np.uint8,
            )
            # img = cv2.cvtColor(np.asarray(img), cv2.COLOR_RGB2BGR)
        ...
        for i in range(272):
            loss_lm += torch.mean(
                torch.abs(
                    torch.tensor(
                        (
                            (fake_row[i * 2] - real_row[i * 2]) ** 2
                            + (fake_row[i * 2 + 1] - real_row[i * 2 + 1]) ** 2
                        )
                        ** 1
                        / 2
                    )
    ...
    if args.style_cyc:
        ...
        if args.pix2pix:
            ...
            loss = (
                loss_adv
                + args.lambda_pixel * loss_pixel_1
                + args.lambda_id * (loss_id + loss_id_2)
                + args.lambda_style_cyc * loss_id_cyc
        ...
        return loss, Munch(
            adv=loss_adv.item(), pixel_1=loss_pixel_1.item(), id=loss_id.item()
        )
