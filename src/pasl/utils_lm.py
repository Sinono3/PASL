...
@torch.no_grad()
def translate_and_reconstruct_multi(
    nets,
    args,
    x1_1_source,
    x1_2_source,
    ...
    masks = None
    x_fake = nets.generator(
        x9_target_lm, s_ref_mean, masks=masks, loss_select=args.loss
    )

    x_concat = [x9_target_lm, x1_1_source, x9_target, x_fake[:, [0, 1, 2], :, :]]
...
def translate_and_reconstruct(
    nets, args, x1, x1_lm, x2_target, x2_target_lm, lm, filename
):
    # def translate_and_reconstruct(nets, args, x1, x2_target, x2_target_lm, filename):
    N, C, H, W = x1.size()

    s_ref = nets.style_encoder(x1)

    masks = None
    x_fake = nets.generator(x2_target_lm, lm, s_ref, masks=masks)

    x_concat = [x2_target_lm, lm, x1, x_fake[:, [0, 1, 2], :, :], x2_target]
...
def debug_image(nets, args, inputs, step):
    if args.multi:
        ...
        translate_and_reconstruct_multi(
            nets,
            args,
            x1_1_source,
            x1_2_source,
