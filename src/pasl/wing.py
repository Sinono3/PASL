...
class AddCoordsTh(nn.Module):
    def __init__(self, height=64, width=64, with_r=False, with_boundary=False):
        ...
        with torch.no_grad():
            ...
            if self.with_r:
                rr = torch.sqrt(
                    torch.pow(x_coords, 2) + torch.pow(y_coords, 2)
                )  # (height, width)
    ...
    def forward(self, x, heatmap=None):
        ...
        if self.with_boundary and heatmap is not None:
            ...
            xx_boundary_channel = torch.where(
                boundary_channel > 0.05, self.x_coords, zero_tensor
            ).to(zero_tensor.device)
            yy_boundary_channel = torch.where(
                boundary_channel > 0.05, self.y_coords, zero_tensor
            ).to(zero_tensor.device)
            coords = torch.cat(
...
class ConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes):
        ...
        conv3x3 = partial(
            nn.Conv2d, kernel_size=3, stride=1, padding=1, bias=False, dilation=1
        )
...
class FAN(nn.Module):
    ...
    def load_pretrained_weights(self, fname):
        ...
        model_weights.update(
            {k: v for k, v in checkpoint["state_dict"].items() if k in model_weights}
        )
    ...
    def get_heatmap(self, x, b_preprocess=True):
        ...
        if b_preprocess:
            heatmaps = F.interpolate(
                heatmaps, scale_factor=scale_factor, mode="bilinear", align_corners=True
            )
    ...
    def align(self, imgs, output_size=256):
        ...
        for i, (img, landmarks) in enumerate(zip(imgs, landmarkss)):
            ...
            aligned = cv2.warpPerspective(
                img_np, transform, (cols, rows), flags=cv2.INTER_LANCZOS4
            )
...
def preprocess(x):
    ...
    x[:, start:end] = resize(x[:, start:end], 1)
    x[:, start:end] = resize(shift(x[:, start:end], -8), 3) + shift(
        x[:, start:end], -24
    )
