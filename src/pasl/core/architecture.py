...
class Generator(nn.Module):
    def __init__(self, img_size=256, style_dim=64, max_conv_dim=512, w_hpf=1):
        ...
        encoder_layer = TransformerEncoderLayer(
            TN_HIDDEN_DIM, TN_NHEADS, TN_DIM_FEEDFORWARD, TN_DROPOUT, "relu", True
        )
        encoder_norm = nn.LayerNorm(TN_HIDDEN_DIM) if True else None
        self.trands_encoder = TransformerEncoder(
            encoder_layer, TN_ENC_LAYERS, encoder_norm
        )
        ...
        self.trans_decoder = TransformerDecoder(
            decoder_layer, TN_DEC_LAYERS, decoder_norm, return_intermediate=True
        )
    ...
    def forward(self, x, lm, s, masks=None):
        ...
        hs_sk = hs_sk.transpose(1, 2)[
            -1
        ]  # torch.cat([hs.transpose(1, 2)[-1], QR_EMB.permute(1,0,2)], -1)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(
        ...
        value = tgt2
        tgt2 = self.self_attn(
            q, k, value, attn_mask=tgt_mask, key_padding_mask=tgt_key_padding_mask
        )[0]
        tgt = tgt + self.dropout1(tgt2)
        tgt2 = self.norm2(tgt)
        tgt2 = self.multihead_attn(
...
class Discriminator_img2_pix(nn.Module):
    def __init__(self, img_size=256, num_domains=2, self_att=False, max_conv_dim=512):
        ...
        self.output = nn.Sequential(
            nn.LeakyReLU(0.2),
            nn.Conv2d(dim_out, dim_out, 2, 1, 0),
            nn.LeakyReLU(0.2),
            nn.Conv2d(dim_out, 1, 1, 1, 0),
        )
