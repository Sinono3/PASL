...
parser.add_argument(
    "--hidden_dim", type=int, default=512, help="Hidden dimension of mapping network"
)
parser.add_argument(
...
parser.add_argument("--style_dim", type=int, default=128, help="Style code dimension")
# loss select
parser.add_argument(
...
parser.add_argument(
    "--self_att", action="store_true", default=True, help="use self-attention"
)
parser.add_argument(
...
def fuse_shape(deca, face_detector, src_codedict, ref_img):
    testdata = datasets_demo.TestData(
        face_detector, [ref_img], iscrop=True, face_detector="fan", sample_step=10
    )
    device = "cuda"
    i = 0
    depth_image_list = []
...
def get_codedict(deca, face_detector, img_list):
    testdata = datasets_demo.TestData(
        face_detector, img_list, iscrop=True, face_detector="fan", sample_step=10
    )
    device = "cuda"
    i = 0
    deca_cfg.model.use_tex = False
        ...
        if idx % freq == 0:
            ...
            if preds is not None:
                ...
                try:
                    ...
                    depth_image_list, lm_image_list = get_shape(
                        deca, face_detector, np.array(img_crop2)
                    )
                ...
                all_result[:256, 256:512, :] = cv2.cvtColor(
                    img_fake_2, cv2.COLOR_RGB2BGR
                )
                all_result[256:512, :256, :] = cv2.cvtColor(
                    img_fake_3, cv2.COLOR_RGB2BGR
                )
                all_result[256:512, 256:512, :] = cv2.cvtColor(
                    img_fake_4, cv2.COLOR_RGB2BGR
                )
                ...
                combined_result = np.hstack(
                    (frame_resized, source_image_resized, all_result)
                )
