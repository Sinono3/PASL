...
from pasl.model_lm_talking import build_model
from pasl.checkpoint import CheckpointIO
...
class Solver(nn.Module):
    def __init__(self, args):
        ...
        self.ckptios = [
            CheckpointIO(
                ospj(args.checkpoint_dir, "{}_nets_ema.ckpt".format(args.resume_iter)),
                **self.nets_ema,
            )
        ]
        self.half()  # modify
...
def drawshape(landmarks):
    ...
    if diff <= 30 and int(float(landmarks[30][0])) > int(float(landmarks[42][0])):
        for n in range(0, 12):
            cv2.line(
                img,
                (int(float(landmarks[n][0])), int(float(landmarks[n][1]))),
                (int(float(landmarks[n + 1][0])), int(float(landmarks[n + 1][1]))),
                line_color,
                line_width,
            )
        ...
        cv2.line(
            img,
            (int(float(landmarks[36][0])), int(float(landmarks[36][1]))),
            (int(float(landmarks[41][0])), int(float(landmarks[41][1]))),
            line_color,
            line_width,
        )
        ...
        cv2.line(
            img,
            (int(float(landmarks[51][0])), int(float(landmarks[51][1]))),
            (int(float(landmarks[62][0])), int(float(landmarks[62][1]))),
            line_color,
            line_width,
        )
        ...
        cv2.line(
            img,
            (int(float(landmarks[60][0])), int(float(landmarks[60][1]))),
            (int(float(landmarks[67][0])), int(float(landmarks[67][1]))),
            line_color,
            line_width,
        )
        cv2.line(
            img,
            (int(float(landmarks[67][0])), int(float(landmarks[67][1]))),
            (int(float(landmarks[66][0])), int(float(landmarks[66][1]))),
            line_color,
            line_width,
        )
        cv2.line(
            img,
            (int(float(landmarks[57][0])), int(float(landmarks[57][1]))),
            (int(float(landmarks[66][0])), int(float(landmarks[66][1]))),
            line_color,
            line_width,
        )
        ...
        cv2.line(
            img,
            (int(float(landmarks[48][0])), int(float(landmarks[48][1]))),
            (int(float(landmarks[59][0])), int(float(landmarks[59][1]))),
            line_color,
            line_width,
        )
        ...
        cv2.line(
            img,
            (int(float(landmarks[42][0])), int(float(landmarks[42][1]))),
            (int(float(landmarks[47][0])), int(float(landmarks[47][1]))),
            line_color,
            line_width,
        )
...
def turn_eye(fake_lm, x2_label):
    ...
    real_left_eye_dis = torch.abs(
        (
            (x2_label[:, 2 * 36 : 2 * 36 + 1] - x2_label[:, 2 * 39 : 2 * 39 + 1]) ** 2
            + (
                x2_label[:, 2 * 36 + 1 : 2 * 36 + 2]
                - x2_label[:, 2 * 39 + 1 : 2 * 39 + 2]
            )
            ** 2
        )
        ** (0.5)
    ...
    fake_left_eye_dis = torch.abs(
        (
            (fake_lm[:, 2 * 36 : 2 * 36 + 1] - fake_lm[:, 2 * 39 : 2 * 39 + 1]) ** 2
            + (
                fake_lm[:, 2 * 36 + 1 : 2 * 36 + 2]
                - fake_lm[:, 2 * 39 + 1 : 2 * 39 + 2]
            )
            ** 2
        )
        ** (0.5)
    ...
    real_left_eye_1_x = (
        x2_label[:, 2 * 37 : 2 * 37 + 1] - x2_label[:, 2 * 41 : 2 * 41 + 1]
    )
    real_left_eye_1_y = (
        x2_label[:, 2 * 37 + 1 : 2 * 37 + 2] - x2_label[:, 2 * 41 + 1 : 2 * 41 + 2]
    )

    real_left_eye_2_x = (
        x2_label[:, 2 * 38 : 2 * 38 + 1] - x2_label[:, 2 * 40 : 2 * 40 + 1]
    )
    real_left_eye_2_y = (
        x2_label[:, 2 * 38 + 1 : 2 * 38 + 2] - x2_label[:, 2 * 40 + 1 : 2 * 40 + 2]
    )

    real_right_eye_1_x = (
        x2_label[:, 2 * 43 : 2 * 43 + 1] - x2_label[:, 2 * 47 : 2 * 47 + 1]
    )
    real_right_eye_1_y = (
        x2_label[:, 2 * 43 + 1 : 2 * 43 + 2] - x2_label[:, 2 * 47 + 1 : 2 * 47 + 2]
    )

    real_right_eye_2_x = (
        x2_label[:, 2 * 44 : 2 * 44 + 1] - x2_label[:, 2 * 46 : 2 * 46 + 1]
    )
    real_right_eye_2_y = (
        x2_label[:, 2 * 44 + 1 : 2 * 44 + 2] - x2_label[:, 2 * 46 + 1 : 2 * 46 + 2]
    )

    for i in range(0, N):
        fake_lm[i, 2 * 37 : 2 * 37 + 1] = (
            fake_lm[i, 2 * 41 : 2 * 41 + 1] + real_left_eye_1_x[i] * ratio[i]
        )
        fake_lm[i, 2 * 37 + 1 : 2 * 37 + 2] = (
            fake_lm[i, 2 * 41 + 1 : 2 * 41 + 2] + real_left_eye_1_y[i] * ratio[i]
        )

        fake_lm[i, 2 * 38 : 2 * 38 + 1] = (
            fake_lm[i, 2 * 40 : 2 * 40 + 1] + real_left_eye_2_x[i] * ratio[i]
        )
        fake_lm[i, 2 * 38 + 1 : 2 * 38 + 2] = (
            fake_lm[i, 2 * 40 + 1 : 2 * 40 + 2] + real_left_eye_2_y[i] * ratio[i]
        )

        fake_lm[i, 2 * 43 : 2 * 43 + 1] = (
            fake_lm[i, 2 * 47 : 2 * 47 + 1] + real_right_eye_1_x[i] * ratio[i]
        )
        fake_lm[i, 2 * 43 + 1 : 2 * 43 + 2] = (
            fake_lm[i, 2 * 47 + 1 : 2 * 47 + 2] + real_right_eye_1_y[i] * ratio[i]
        )

        fake_lm[i, 2 * 44 : 2 * 44 + 1] = (
            fake_lm[i, 2 * 46 : 2 * 46 + 1] + real_right_eye_2_x[i] * ratio[i]
        )
        fake_lm[i, 2 * 44 + 1 : 2 * 44 + 2] = (
            fake_lm[i, 2 * 46 + 1 : 2 * 46 + 2] + real_right_eye_2_y[i] * ratio[i]
        )
    return fake_lm
    ...
    for i in range(0, N):
        ...
        if diff_1 <= 30 and int(float(lm_x[30])) > int(float(lm_x[42])):
            for n in range(0, 12):
                cv2.line(
                    img_3,
                    (int(float(lm_x[n])), int(float(lm_y[n]))),
                    (int(float(lm_x[n + 1])), int(float(lm_y[n + 1]))),
                    line_color,
                    line_width,
                )
            for n in range(17, 21):
            ...
            cv2.line(
                img_3,
                (int(float(lm_x[36])), int(float(lm_y[36]))),
                (int(float(lm_x[41])), int(float(lm_y[41]))),
                line_color,
                line_width,
            )
            for n in range(48, 51):
            ...
            cv2.line(
                img_3,
                (int(float(lm_x[51])), int(float(lm_y[51]))),
                (int(float(lm_x[62])), int(float(lm_y[62]))),
                line_color,
                line_width,
            )
            for n in range(60, 62):
            ...
            cv2.line(
                img_3,
                (int(float(lm_x[60])), int(float(lm_y[60]))),
                (int(float(lm_x[67])), int(float(lm_y[67]))),
                line_color,
                line_width,
            )
            cv2.line(
                img_3,
                (int(float(lm_x[66])), int(float(lm_y[66]))),
                (int(float(lm_x[67])), int(float(lm_y[67]))),
                line_color,
                line_width,
            )
            cv2.line(
                img_3,
                (int(float(lm_x[57])), int(float(lm_y[57]))),
                (int(float(lm_x[66])), int(float(lm_y[66]))),
                line_color,
                line_width,
            )
            ...
            cv2.line(
                img_3,
                (int(float(lm_x[48])), int(float(lm_y[48]))),
                (int(float(lm_x[59])), int(float(lm_y[59]))),
                line_color,
                line_width,
            )
            ...
            cv2.line(
                img_3,
                (int(float(lm_x[42])), int(float(lm_y[42]))),
                (int(float(lm_x[47])), int(float(lm_y[47]))),
                line_color,
                line_width,
            )
            for n in range(51, 57):
...
def main(args):
    ...
    for i, frame in tqdm(enumerate(reader)):
        ...
        if i == 0:
            ...
            try:
                param_lst, roi_box_lst = tddfa(
                    img_crop2, [pre_ver], crop_policy="landmark"
                )
            except:
                continue
        ...
        if len(queue_ver) >= n:
            ...
            pncc_map[2, :, :, :] = img_draw
            pncc_map[3, :, :, :] = img_draw
            img_fake, img_fake_2, img_fake_3, img_fake_4 = solver.sample(
                source_style_code, pncc_map, lm_shape
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="pncc",
        choices=["2d_sparse", "2d_dense", "3d", "pncc"],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--id_embed_dim", type=int, default=4096, help="ID code dimension by VGGFace"
    )
    parser.add_argument(
        "--id_embed", action="store_true", default=False, help="multi input"
    )
    # style code dimension
    parser.add_argument(
        "--style_dim", type=int, default=128, help="Style code dimension"
    )
    # loss select
    parser.add_argument(
        "--loss", type=str, default="perceptual", help="the type of loss. [perceptual]"
    )
    # dataset select
    parser.add_argument(
        "--pix2pix", action="store_true", default=False, help="use pix2pix loss"
    )
    parser.add_argument(
        "--multi_discriminator",
        action="store_true",
    ...
    parser.add_argument(
        "--masks", action="store_true", default=False, help="use mask injection"
    )
    parser.add_argument(
        "--self_att", action="store_true", default=False, help="use self-attention"
    )
    parser.add_argument(
